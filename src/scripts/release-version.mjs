...
export function compareSemver(leftVersion, rightVersion) {
  ...
  if (!left || !right)
    throw new TypeError("Cannot compare invalid semantic versions.");
...
export function latestOpenSeTag(tags) {
  const releases = tags.filter(
    (tag) =>
      tag.startsWith("opense-v") && parseSemver(tag.slice("opense-v".length)),
  );
  return (
    releases.sort((left, right) =>
      compareSemver(
        right.slice("opense-v".length),
        left.slice("opense-v".length),
      ),
    )[0] ?? null
  );
}
