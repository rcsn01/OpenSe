...
const installBridge = (overrides: Partial<OpenPiBridge> = {}) => {
...
describe('AssistantWorkspace', () => {
  beforeEach(() => {
    vi.restoreAllMocks()
    Reflect.deleteProperty(window, 'openPi')
  })
  ...
  it('opens a clicked session row and renders its emitted history', async () => {
    const openSession = vi.fn(async (sessionId: string) => {
      ;(window.openPi as (OpenPiBridge & { emit: (event: AssistantSessionEvent) => void })).emit({
        type: 'messages',
        sessionId,
        messages: [
          {
