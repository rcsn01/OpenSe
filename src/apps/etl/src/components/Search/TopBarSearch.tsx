...
const getRouteFallbackSearchConfig = (pathname: string): PageTopBarSearchConfig | null => {
  if (pathname === '/dashboard' || pathname.startsWith('/dashboard/')) {
    return { searchKey: 'dashboard-workflows', placeholder: 'Search workflows...' };
  }

  if (pathname === '/gallery' || pathname.startsWith('/gallery/')) {
    return { searchKey: 'gallery-templates', placeholder: 'Search templates...' };
  }

  if (pathname === '/organisation' || pathname.startsWith('/organisation/team')) {
    return { searchKey: 'organisation-team', placeholder: 'Search members...' };
  }

  if (pathname === '/activity' || pathname.startsWith('/activity/')) {
    return { searchKey: 'activity', placeholder: 'Search activity...' };
  }
...
export const TopBarSearchProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const activeRegistration = useMemo(
    () => {
      const orderedRegistrations = Array.from(registrations.values()).sort((left, right) => left.order - right.order);
      return orderedRegistrations[orderedRegistrations.length - 1] ?? null;
    },
    [registrations],
  );
  ...
  const registerPageSearch = useCallback((registrationId: string, config: PageTopBarSearchConfig) => {
    setRegistrations((current) => {
      ...
      next.set(
        ...
        existing
          ? { ...existing, config }
          : { config, order: nextOrderRef.current++ },
      );
      return next;
    });
  }, []);
