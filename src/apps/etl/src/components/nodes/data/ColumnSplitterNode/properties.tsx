...
interface ColumnSplitterNodePropertiesProps {
  data: ColumnSplitterNodeData;
  onChange: (key: string, value: any) => void;
}

export const ColumnSplitterNodeProperties: React.FC<ColumnSplitterNodePropertiesProps> = ({ data, onChange }) => {
  ...
  const selectAll = () => onChange('selectedColumns', [...available]);
  const deselectAll = () => onChange('selectedColumns', []);

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <label className="block text-xs font-semibold text-slate-500">Columns for Top Output</label>
        <span className="text-xs text-slate-400">{selected.length} selected</span>
      </div>

      <p className="text-[10px] text-slate-400">
        Selected columns go to the <strong>Top</strong> output. Remaining go to <strong>Bottom</strong>.
      </p>
