...
export function Code({ children, className, block = false }: CodeProps) {
  if (block) {
    return (
      <pre
        className={cn(
          'overflow-x-auto rounded-[var(--radius-lg)] bg-[var(--color-muted)] p-4 text-sm leading-relaxed',
          className,
        )}
      >
        <code>{children}</code>
  ...
  return (
    <code
      className={cn(
        'rounded-[var(--radius-sm)] bg-[var(--color-muted)] px-1.5 py-0.5 text-sm font-mono',
        className,
      )}
    >
      {children}
