...
vi.mock("../../hooks/queries/useProcurementTabs", () => ({
  useProcurementPurchaseOrders: (...args: unknown[]) =>
    mockUseProcurementPurchaseOrders(...args),
  useProcurementPurchaseOrderItems: (...args: unknown[]) =>
    mockUseProcurementPurchaseOrderItems(...args),
}));
    ...
    <MemoryRouter initialEntries={["/dashboard"]}>
      <Routes>
        <Route element={<AppLayout />}>
          ...
          <Route path="/inventory/:id/:tab" element={<LocationDisplay />} />
        </Route>
      </Routes>
    </MemoryRouter>,
  );
...
describe("DashboardPage", () => {
  it("renders the redesigned dashboard sections with live data", () => {
    ...
    expect(screen.getAllByTestId("analytics-comparison-bar")).toHaveLength(14);
  ...
  it("navigates to a product detail page from dashboard search", async () => {
    const user = userEvent.setup();
    renderPageWithLayout();

    const searchInput = screen.getByPlaceholderText("Search items...");
    await user.type(searchInput, "Acrylic");
    await user.keyboard("{Enter}");

    expect(await screen.findByTestId("location-display")).toHaveTextContent(
      "/inventory/p-6/overview",
    );
  });
