...
const buildPermissionActionColumns = (groups: PermissionGroup[]): PermissionActionColumn[] => {
...
export const RolePermissionsEditPage = () => {
  ...
  const handleTogglePermissionGroup = (group: PermissionGroup, permissionCodes: string[], checked: boolean) => {
    setSelectedPermissions((current) => {
      const permissionCodeSet = new Set(permissionCodes)

      if (checked) {
        return Array.from(new Set([...current, ...(group.viewCode ? [group.viewCode] : []), ...permissionCodes]))
      }
                        ...
                        >
                          <span className="mx-auto flex max-w-20 flex-col items-center gap-0.5 whitespace-normal">
                            {splitPermissionActionLabel(column.label).map((part, index) => (
                              <span key={`${column.key}-${part}-${index}`}>{part}</span>
                            ))}
                          </span>
                        </TableHead>
                      ))}
                  ...
                  <TableBody>
                    {permissionGroups.map((group) => (
                      <TableRow key={group.key}>
                        ...
                        {permissionActionColumns.map((column) => {
                          const cellPermissions = group.permissions.filter((item) => item.columnKey === column.key)
                          const cellPermissionCodes = cellPermissions.map((permission) => permission.code)
                          const isCellChecked = cellPermissionCodes.length > 0
                            && cellPermissionCodes.every((code) => selectedPermissions.includes(code))
                          const cellLabel = cellPermissions.length > 1
                            ? `${group.label}: ${column.label}`
                            : `${group.label}: ${cellPermissions[0]?.label ?? column.label}`
                          const cellTitle = cellPermissions.map((permission) => permission.label).join(', ')
                          ...
                          return (
                            ...
                            >
                              {cellPermissions.length > 0 ? (
                                  ...
                                  <Checkbox
                                    ...
                                    onChange={(event) => {
                                      if (cellPermissions.length === 1) {
                                        handleTogglePermission(cellPermissions[0].code, event.target.checked)
                                        return
                                      }

                                      handleTogglePermissionGroup(group, cellPermissionCodes, event.target.checked)
                                    }}
                                ...
                                <span className="text-[var(--color-muted-foreground)]/45" aria-hidden="true">
                                  -
                                </span>
                              )}
                            </TableCell>
