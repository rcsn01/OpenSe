...
interface Toast {
  id: string; message: string; title?: string; variant: ToastVariant; duration?: number
}
...
const variantIcons: Record<ToastVariant, typeof Info> = {
  default: Info, success: CheckCircle2, destructive: AlertCircle, warning: AlertTriangle, info: Info,
}

/* ── Context ──────────────────────────────────────────── */

interface ToastContextValue {
  toast: (opts: { message: string; title?: string; variant?: ToastVariant; duration?: number }) => void
}
...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  const toast = useCallback(({ message, title, variant = 'default', duration = 4000 }: { message: string; title?: string; variant?: ToastVariant; duration?: number }) => {
    ...
    <ToastContext.Provider value={{ toast }}>
      ...
      <div className="fixed bottom-4 right-4 z-[100] flex flex-col gap-2 max-w-sm">
        {toasts.map((t) => {
          ...
          return (
            <div key={t.id} className={cn('flex items-start gap-3 rounded-[var(--radius-lg)] border p-4 shadow-[var(--shadow-lg)] animate-in slide-in-from-right', variantStyles[t.variant])}>
              <Icon className="mt-0.5 h-4 w-4 shrink-0" />
              <div className="flex-1 min-w-0">
                {t.title && <p className="font-medium text-sm">{t.title}</p>}
                <p className="text-sm">{t.message}</p>
              </div>
              <button onClick={() => dismiss(t.id)} className="shrink-0 opacity-70 hover:opacity-100"><X className="h-4 w-4" /></button>
            </div>
          )
        })}
      </div>
