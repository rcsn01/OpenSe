...
export interface LandingNavbarProps {
  ...
  actions?: ReactNode
  renderLink?: (link: LandingNavbarLink, options: LandingNavbarRenderLinkOptions) => ReactNode
  mobileMenu?: LandingNavbarMobileMenu
}

const renderAnchor = (link: LandingNavbarLink, options: LandingNavbarRenderLinkOptions) => (
  <a
...
export const LandingNavbar = forwardRef<HTMLElement, LandingNavbarProps>(function LandingNavbar(
  {
    as = 'div',
    brand,
    segmentedLinks,
  ...
  const rootStyle: CSSProperties = {
    ...
    backgroundColor: isScrolled ? 'color-mix(in srgb, var(--color-card) 88%, transparent)' : 'transparent',
  ...
  const segmentedGroupClassName = [
    ...
    isScrolled
      ? 'border border-[var(--color-border)] bg-white/75 text-[var(--color-muted-foreground)]'
  ...
  const mobilePanelStyle: CSSProperties = {
    ...
    boxShadow: '0 14px 40px color-mix(in srgb, var(--color-foreground) 10%, transparent)',
    ...
    >
      ...
      <div className="hidden items-center gap-8 text-sm font-medium md:flex">
        {segmentedLinks?.length ? (
          <div aria-label={segmentedLinksAriaLabel} className={segmentedGroupClassName}>
            {segmentedLinks.map((link) => renderLink(link, { className: segmentedLinkClassName }))}
          </div>
        ) : null}
      ...
      {mobileMenu?.open ? (
        ...
        >
          {mobileMenu.items.map((link) =>
            renderLink(link, {
              className: 'text-sm font-medium px-3 py-2 transition-transform duration-200 hover:-translate-y-px',
              onClick: mobileMenu.onToggle,
            }),
          )}
          {mobileMenu.action}
