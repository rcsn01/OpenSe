...
test.describe('Stoqr Procurement', () => {
  test('procurement page loads', async ({ authenticatedPage }) => {
    ...
    await authenticatedPage.getByRole('tab', { name: /vendor returns/i }).click();
    await expect(authenticatedPage.getByRole('button', { name: /log new return/i })).toBeVisible();
    await expect(authenticatedPage.getByText(/credit \/ refund/i).first()).toBeVisible();
