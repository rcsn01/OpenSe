...
BEGIN
  ...
  IF NOT public.has_permission(v_company_id, 'inventory.view') THEN
    RAISE EXCEPTION 'Viewer should have inventory.view';
  END IF;

  IF NOT public.has_permission(v_company_id, 'inventory.use') THEN
    RAISE EXCEPTION 'Viewer inventory.view legacy backfill should imply inventory.use';
  END IF;

  IF public.has_permission(v_company_id, 'inventory.adjust') THEN
    RAISE EXCEPTION 'Viewer should not have inventory.adjust';
  END IF;

  PERFORM set_config('request.jwt.claim.sub', '33333333-3333-3333-3333-333333333333', true);

  IF NOT public.has_permission(v_company_id, 'scanner.view') THEN
    RAISE EXCEPTION 'Manager scanner.use should imply scanner.view';
  END IF;

  IF NOT public.has_permission(v_company_id, 'procurement.receive') THEN
    RAISE EXCEPTION 'Manager should have procurement.receive from seeded role';
  END IF;

  PERFORM set_config('request.jwt.claim.sub', '11111111-1111-1111-1111-111111111111', true);

  IF NOT public.has_permission(v_company_id, 'organisation.billing.manage') THEN
