-- Cycles are discovered from List grouping and opened as route-driven detail
-- panes/pages. Existing project tab rows are soft-deleted so saved tab bars
-- no longer render a standalone Cycles tab.

UPDATE open_kb.project_tabs
SET deleted_at = COALESCE(deleted_at, timezone('utc'::text, now()))
WHERE tab_key = 'cycles'
...
BEGIN
  ...
  IF NEW.tab_key IN ('drafts', 'modules', 'cycles') AND NEW.deleted_at IS NULL THEN
