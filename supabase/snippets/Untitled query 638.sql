...
CREATE INDEX IF NOT EXISTS logs_org_created_idx     ON etl.logs (org_id, created_at DESC);
CREATE INDEX IF NOT EXISTS logs_category_status_idx ON etl.logs (event_category, status);
CREATE INDEX IF NOT EXISTS logs_severity_idx        ON etl.logs (severity) WHERE severity IN ('error', 'critical');
...
GRANT SELECT ON etl.logs TO authenticated;
GRANT EXECUTE ON FUNCTION etl.log_event TO authenticated;