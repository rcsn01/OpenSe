...
const getFeatureFlags = async (supabaseUrl: string, serviceRoleKey: string, organisationId: string) =>
  await restFetch<Array<{ slack_sync_enabled: boolean }>>(
    supabaseUrl,
    serviceRoleKey,
...
const upsertIntegration = async (
  ...
  const existing = await restFetch<OrganisationIntegrationRow[]>(
    supabaseUrl,
    serviceRoleKey,
    `organisation_integrations?organisation_id=eq.${encodeURIComponent(payload.organisation_id)}&provider=eq.slack&deleted_at=is.null&select=id`,
  )
  const row = {
...
Deno.serve(async (req: Request) => {
  ...
  const clientId = Deno.env.get('OPEN_KB_SLACK_CLIENT_ID')
  const clientSecret = Deno.env.get('OPEN_KB_SLACK_CLIENT_SECRET')
  const stateSecret = Deno.env.get('OPEN_KB_OAUTH_STATE_SECRET')
  const redirectUri = Deno.env.get('OPEN_KB_SLACK_REDIRECT_URI')
  ...
  if (!supabaseUrl || !serviceRoleKey || !anonKey || !clientId || !clientSecret || !stateSecret || !redirectUri || !credentialKey) {
    return json(503, { error: 'Slack OAuth is not configured' })
  }
  ...
  if (req.method === 'POST') {
    ...
    if (!flags[0]?.slack_sync_enabled) return json(403, { error: 'Slack sync is disabled' })
    ...
    url.searchParams.set('scope', 'channels:read,chat:write')
  ...
  if (req.method === 'GET') {
    ...
    if (!code || !state) return json(400, { error: 'Missing Slack OAuth code or state' })
    ...
    if (!flags[0]?.slack_sync_enabled) return redirect(`${verifiedState.return_to}?openKbIntegration=slack_disabled`)
    ...
    if (!tokenResponse.ok || !token.ok || !token.access_token) {
      throw new Error(token.error ?? 'Slack OAuth token exchange failed')
    }
    ...
    const organisationIntegrationId = await upsertIntegration(supabaseUrl, serviceRoleKey, {
      ...
      external_account_id: token.team?.id ?? null,
    })
    if (!organisationIntegrationId) throw new Error('Slack integration row was not created')
    ...
    await upsertIntegrationCredential(supabaseUrl, serviceRoleKey, {
      ...
      metadata: {
        team_id: token.team?.id ?? null,
        team_name: token.team?.name ?? null,
        authed_user_id: token.authed_user?.id ?? null,
        scopes: (token.scope ?? '').split(',').map((scope) => scope.trim()).filter(Boolean),
      },
    })

    return redirect(`${verifiedState.return_to}?openKbIntegration=slack_connected`)
  }
