...
// replacements now handle substrings (e.g. useOrganisationMembers) and preserve casing
...
const replacements = [
  ...
  { from: /organisations/ig, to: function(m) { return preserveCase(m, 'organisations'); } },
  { from: /organisation/ig, to: function(m) { return preserveCase(m, 'organisation'); } },
];
...
function walk(dir) {
  ...
  entries.forEach(entry => {
    ...
    if (entry.isDirectory()) {
      walk(full);
      // rename directory if contains 'organisation' (case-insensitive)
      if (/organisation/i.test(entry.name)) {
        const newName = entry.name
          .replace(/organisations/ig, (m) => preserveCase(m, 'organisations'))
          .replace(/organisation/ig, (m) => preserveCase(m, 'organisation'));
      ...
      processFile(full);
      // rename file if contains 'organisation' (case-insensitive)
      if (/organisation/i.test(entry.name)) {
        const newName = entry.name
          .replace(/organisations/ig, (m) => preserveCase(m, 'organisations'))
          .replace(/organisation/ig, (m) => preserveCase(m, 'organisation'));
